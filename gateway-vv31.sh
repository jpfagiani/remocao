...
if [ "$WAN_MODE" = "dhcp" ]; then
    warn "Modo DHCP selecionado para WAN."
    warn "Um gateway exige IP fixo para funcionar de forma confiável."
    warn "Os serviços (BIND9, Squid, nftables) usarão o IP atual no momento da instalação."
    warn "Se o IP mudar após reboot, reconfigure manualmente ou use IP Reservation no roteador."
    echo ""
...
{
cat <<NETEOF
# Gerado automaticamente pelo gateway-v27 em $(date '+%Y-%m-%d %H:%M:%S')
# WAN_MODE=${WAN_MODE}
...
# Problema: ao parar NetworkManager (seção anterior), o /etc/resolv.conf pode
# virar symlink quebrado ou apontar para 127.0.0.53 (stub do resolved parado),
# fazendo o apt falhar por não conseguir resolver nomes de servidores.
# Solução: forçar DNS público confiável agora; os internos são configurados
# depois que o BIND9 estiver instalado e rodando (seção 5).
...
if [ -L /etc/resolv.conf ] || [ ! -s /etc/resolv.conf ]; then
    ...
    rm -f /etc/resolv.conf
    # IMPORTANTE: usar DNS públicos aqui — BIND9 ainda não está instalado
    # Os DNS internos são configurados após o BIND9 subir (seção 5)
...
# ─── Instalar ifupdown ANTES do apt-get update completo ──────────────────────
# Necessário porque a seção de rede já gravou /etc/network/interfaces
# e pode tentar usar ifup. Instala agora com a lista de pacotes atual.
...
# apt-get update: tentar até 3 vezes (falhas transitórias de rede são comuns)
# Usar || true + verificação manual para não abortar o script inteiro no set -e
APT_UPDATE_OK=false
...
# Debian 13: libpam0g-dev renomeado para libpam-dev, bind9-doc separado
# python3-pip bloqueado por PEP 668 — usar apenas venv (já é a prática correta)
# conntrack: pacote dividido em conntrack + nftables já inclui o necessário
DEBIAN_VER=$(. /etc/os-release; echo "${VERSION_ID:-12}")
PAM_DEV_PKG="libpam0g-dev"
[ "${DEBIAN_VER}" -ge 13 ] 2>/dev/null && PAM_DEV_PKG="libpam-dev"

# Instalar pacotes base — primeiro tenta com ifupdown2 (Debian 13),
# se falhar tenta com ifupdown (Debian 12 e anteriores).
# O || true final evita que o script aborte por pacote não encontrado.
...
if apt-cache show squid-openssl &>/dev/null 2>&1; then
    ...
    if squid -v 2>&1 | grep -qi 'with-openssl\|enable-ssl\|ssl' ||        [ -f /usr/lib/squid/security_file_certgen ] ||        [ -f /usr/lib/squid/ssl_crtd ]; then
...
fs.file-max=2097152
EOF
# Garantir que módulo nf_conntrack está carregado antes de aplicar sysctl
...
if [ "${MON_ENABLED:-0}" = "1" ]; then
    ...
    ip addr add ${MON_IP}/24 dev "$LAN_IF" label "${LAN_IF}:mon" 2>/dev/null ||         warn "Alias ja existe ou erro — verifique: ip addr show $LAN_IF"
...
# Alias de monitoramento — gerado pelo gateway
auto ${LAN_IF}:mon
iface ${LAN_IF}:mon inet static
    address ${MON_IP}
    netmask 255.255.255.0
    # Garantir que a rede de monitoramento usa o gateway como rota default
...
MONEOF
    ...
    grep -q 'proxy_arp' /etc/sysctl.d/99-gateway.conf 2>/dev/null ||         echo -e "net.ipv4.conf.all.proxy_arp=1
net.ipv4.conf.${LAN_IF}.proxy_arp=1"         >> /etc/sysctl.d/99-gateway.conf
fi
...
systemctl enable --force "$CHRONY_SVC" 2>/dev/null ||     systemctl enable "$CHRONY_SVC" 2>/dev/null ||     true
systemctl restart "$CHRONY_SVC" 2>/dev/null ||     systemctl start "$CHRONY_SVC" 2>/dev/null ||     warn "Chrony não iniciou — verifique: systemctl status $CHRONY_SVC"
...
# Calcular zona reversa dinamicamente a partir do LAN_IP
# Ex: 192.168.0.x → "0.168.192"; 10.0.1.x → "1.0.10"
...
options {
    ...
    // Forwarders padrão — substituídos por view quando usamos view statements
    // (aqui serve de fallback global)
    forwarders { 10.14.8.20; 10.14.8.16; 10.1.6.222; 8.8.8.8; 1.1.1.1; };
    forward only;
    recursion yes;
...
# O pacote bind9 do Debian instala named.conf.root-hints com a zona "." (root hints)
# no nível global. Quando se usa 'view', TODAS as zonas devem estar dentro de views
# — incluindo root hints e qualquer zona de named.conf.default-zones —
# senão o BIND9 falha com:
#   "when using 'view' statements, all zones must be in views"
# Solução: esvaziar esses arquivos (zona "." será declarada dentro de cada view).
...
# named.conf.default-zones também pode conter zonas globais (localhost, 127.in-addr.arpa)
# que conflitam com o uso de views — neutralizar também.
...
# IMPORTANTE: quando se usa 'view' no BIND9, TODAS as zonas devem estar
# dentro de views — não podem existir zonas no nível global.
# Solução: duas views — "wan_view" para 10.14.29.0/24 e "lan_view" para LAN.
# A wan_view usa forwarders exclusivos; a lan_view contém todas as zonas locais.
# Ambas incluem zona "." (root hints) e zonas localhost para substituir os arquivos padrão.
...
view "wan_view" {
    ...
    recursion yes;
    forwarders { 10.14.8.20; 10.1.6.222; };
    forward only;
    allow-query { rede_wan; };
    allow-query-cache { rede_wan; };

    // Zona root hints — obrigatória em cada view quando named.conf.root-hints é neutralizado
    zone "." {
        type hint;
...
// =============================================================================
// View LAN: loopback + redes LAN + rede de monitoramento
// =============================================================================
view "lan_view" {
    ...
    // Zonas localhost — necessárias quando named.conf.default-zones é neutralizado
    zone "localhost" {
        type master;
...
systemctl enable --force "$NAMED_SVC" 2>/dev/null || systemctl enable "$NAMED_SVC" 2>/dev/null || true
...
# SSL Bump: política por etapa
acl step1 at_step SslBump1
acl step2 at_step SslBump2
acl step3 at_step SslBump3

# Sites que nunca devem ter bump (bancos, gov — evita problemas de HPKP)
...
# via off causa WARNING no Squid 6 - usar httpd_suppress_version_string
httpd_suppress_version_string on
forwarded_for delete
request_header_access X-Forwarded-For deny all

# SSL/TLS — opções de segurança para conexões de saída
# Nota: tls_outgoing_options é usado mesmo sem SSL Bump para proteger conexões upstream
# Se o Squid não tem suporte SSL, esta diretiva é ignorada silenciosamente (Squid 6)
...
# NCSI (Network Connectivity Status Indicator) — detectores de internet
# Windows, Apple e Android usam esses dominios para checar conectividade.
# Sem isso, o icone de rede aparece com X ou triangulo mesmo com internet ok.
...
# 7. IPs parciais: streaming liberado no horário livre, bloqueado fora dele
#    REGRA CRÍTICA DE ORDEM: allow durante horário livre DEVE vir ANTES do deny
#    pois no Squid o primeiro match vence. Se deny vier primeiro, horário livre
#    nunca é avaliado e o streaming fica bloqueado o tempo todo.
#
#    Fluxo para ips_parciais + streaming:
#      a) Horário livre  → allow (streaming liberado)
#      b) Fora do horário → deny streaming_redes / streaming_redes_ssl
#      c) Fora do horário → allow ips_parciais (acesso geral sem streaming)

# 7a. LIBERAR streaming durante horário livre para ips_parciais
...
table inet filter {
    ...
    chain input {
        ...
        # Rede WAN 10.14.29.0/24 — acesso ao DNS do gateway (resolução via 10.14.8.20/10.1.6.222)
    ...
    chain forward {
        ...
        # DNS corporativo: permite que queries DNS dos clientes cheguem a 10.14.8.20/10.1.6.222
        # (após DNAT na prerouting, o pacote já tem daddr=10.14.8.20 e passa aqui)
        ...
        # Bloquear DNS que escapar pelo WAN para destinos que não sejam os DNS corporativos
        # (segurança: impede bypass via DNS-over-HTTPS não interceptado ou VPN DNS)
        ...
        # Tráfego entre LAN principal e rede de monitoramento (NVR/DVR/câmeras)
        # Permite acesso bidirecional entre 192.168.0.x e 192.168.1.x
        ...
        # IPs da rede WAN (10.14.29.x) listados em ips_rede_wan:
        # acesso bidirecional a 192.168.0.0/24 (LAN) e 192.168.1.0/24 (monitoramento)
        ...
        # Rede WAN 10.14.29.0/24 → internet (saída com DNS 10.14.8.20 e 10.1.6.222)
...
table ip nat {
    ...
    chain prerouting {
        ...
        # NCSI HTTPS: redireciona porta 443 para porta 80 do gateway
        # Windows tenta https://www.msftconnecttest.com/connecttest.txt
        # DNS já aponta para ${LAN_IP}; aqui redirecionamos a porta 443→80
        # para o nginx responder corretamente sem SSL bump
        iifname "$LAN_IF" tcp dport 443 ip daddr ${LAN_IP} redirect to :80

        # ── Interceptação de DNS dos clientes LAN ────────────────────────────
        # Objetivo: garantir que os clientes sempre usem os DNS corporativos
        # (10.14.8.20 e 10.1.6.222), que são exigidos por alguns sites internos.
        #
        # Regra: qualquer query DNS (UDP/TCP porta 53) que sair da LAN
        # com destino DIFERENTE do gateway local é redirecionada para 10.14.8.20.
        # Isso impede que clientes usem 8.8.8.8, 1.1.1.1 ou qualquer outro DNS
        # e garante que o DNS leak test dos sites mostre os DNS corporativos.
        #
        # Exceções:
        #   - Destino é o próprio gateway (${LAN_IP}) → deixa ir ao BIND9 local
        #   - Destino já é 10.14.8.20 ou 10.1.6.222 → deixa passar direto
        ...
        # Rede de monitoramento (192.168.1.0/24) — mesma política de DNS
...
# CRÍTICO: restaurar masquerade imediatamente após flush,
# ANTES de processar NAT 1:1 — garante internet mesmo se o resto falhar
...
# Restaurar interceptação de DNS: redireciona queries DNS dos clientes
# para os DNS corporativos 10.14.8.20/10.1.6.222, que são exigidos
# por sites internos que verificam qual DNS o cliente está usando.

# DNS de clientes LAN para destinos externos → redireciona para 10.14.8.20
...
#!/bin/bash
# Encerra horário livre: bloqueia streaming imediatamente
# Estratégia:
#   1. squid -k reconfigure  — aplica ACLs (novas conexões já bloqueadas)
#   2. conntrack cirúrgico   — derruba APENAS conexões dos IPs parciais
#      (derrubar todo 443/80 reconectaria imediatamente via outros IPs)
#   3. Para cada IP parcial, mata conexões TCP ativas porta 80 e 443

TS=$(date '+%Y-%m-%d %H:%M:%S')
echo "[$TS] squid-force-block: encerrando horário livre"

# 1. Reconfigurar Squid — novas requisições já são bloqueadas a partir daqui
...
# 2. Derrubar conexões existentes apenas dos IPs parciais
# Isso força o browser a tentar reconectar, mas o Squid agora nega
...
if command -v conntrack &>/dev/null && [ -f "$PARCIAIS_FILE" ]; then
    while IFS= read -r linha || [ -n "${linha:-}" ]; do
        ...
        # Suporte a CIDR e IP simples
        # conntrack -D --src <IP> derruba todas as conexões originadas desse IP
        ...
        # Fallback sintaxe alternativa Debian
...
# Reconfigure de segurança a cada 5 min — garante que ACLs de horário estão atualizadas
# (proteção contra drift de clock ou cron que não disparou)
...
function FindProxyForURL(url, host) {
    ...
    // Localhost e redes privadas: sem proxy
...
# --- nginx config ---
MON_LISTEN=""
[ "${MON_ENABLED:-0}" = "1" ] && MON_LISTEN="    listen 192.168.1.1:8080;"
...
${MON_LISTEN}
    ...
    # Permitir servir arquivos (disable symlink restriction se necessário)
    disable_symlinks off;

    # Tipos MIME corretos para PAC/DAT
    types {
    ...
    # WPAD automático — browsers buscam /wpad.dat
    location = /wpad.dat {
    ...
    # Download da CA — arquivos copiados diretamente no root (sem alias/symlink)
    # /ca e /ca.der são arquivos físicos em $WPAD_ROOT
    location = /ca {
...
server {
    listen ${LAN_IP}:80;
    listen 192.168.1.1:80;  # rede de monitoramento
...
visudo -c -f /etc/sudoers.d/gateway-panel 2>/dev/null ||     warn "Atenção: sudoers pode ter problema de sintaxe. Verifique /etc/sudoers.d/gateway-panel"

mkdir -p "$PANEL_DIR/templates"

# Debian 13: criar venv com pip incluído explicitamente
python3 -m venv --upgrade-deps "$PANEL_DIR/venv" 2>/dev/null || python3 -m venv "$PANEL_DIR/venv"
# Atualizar pip dentro do venv antes de instalar pacotes
...
"$PANEL_DIR/venv/bin/pip" install --quiet flask    || err "Falha ao instalar flask no venv."
"$PANEL_DIR/venv/bin/pip" install --quiet gunicorn || err "Falha ao instalar gunicorn no venv."
...
# Se python3-pam foi instalado pelo apt, criar symlink no venv para ele encontrar
...
systemctl enable --force gateway-panel 2>/dev/null || systemctl enable gateway-panel 2>/dev/null || true
...
echo -e " ${GREEN}REDE WAN:${NC}  10.14.29.0/24 → internet via ${WAN_IF} (DNS: 10.14.8.20, 10.1.6.222)"
...
echo ""
echo -e "${CYAN}── Funcionalidades do Gateway ──────────────────────────────────────────────${NC}"
echo -e " ${YELLOW}►${NC} IPs Bloqueados agora acessam: gov, bancos, OAB e sites liberados"
echo -e "   ${GREEN}(antes era bloqueio TOTAL — agora é bloqueio parcial inteligente)${NC}"
echo -e " ${YELLOW}►${NC} Microsoft Teams (video + chat) liberado para TODOS os usuários"
echo -e "   ${GREEN}inclusive IPs bloqueados — /etc/squid/sites_teams.txt${NC}"
echo -e " ${YELLOW}►${NC} Lista de bancos brasileiros dedicada — /etc/squid/sites_bancos.txt"
echo -e " ${YELLOW}►${NC} Relatório com botões: Imprimir / Salvar PDF / Salvar HTML"
echo -e " ${YELLOW}►${NC} Relatório com seção dedicada de atividade dos IPs bloqueados"
echo -e "   ${GREEN}(mostra: IP, site, hora, banda, negado vs permitido gov/banco)${NC}"
echo -e " ${YELLOW}►${NC} UI redesenhada: Inter + JetBrains Mono, gradientes, glow effects"
echo -e " ${YELLOW}►${NC} Botões com gradiente, sombra colorida e hover animado"
echo ""
