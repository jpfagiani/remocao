...
if [ "${_VALID_REPOS}" -lt 2 ] 2>/dev/null || [ "$_VALID_REPOS" = "0" ]; then
    warn "Repositórios insuficientes detectados ($_VALID_REPOS). Configurando sources.list oficial..."
...
# ACL para rede WAN 10.14.29.0/24 — usa DNS dedicados 10.14.8.20 e 10.1.6.222
acl "rede_wan" { 10.14.29.0/24; };

options {
    directory "/var/cache/bind";
    # Forwarders padrão (LAN e gateway local)
    forwarders { 10.14.8.20; 10.14.8.16; 10.1.6.222; 8.8.8.8; 1.1.1.1; };
    forward only;
    recursion yes;
    allow-recursion { 127.0.0.1; ${LAN_NET}; 192.168.1.0/24; 10.14.29.0/24; };
    allow-query     { 127.0.0.1; ${LAN_NET}; 192.168.1.0/24; 10.14.29.0/24; };
    allow-transfer  { none; };
    listen-on { 127.0.0.1; $LAN_IP; 192.168.1.1; };
    listen-on-v6 { none; };
    version "none"; hostname "none"; server-id none;
    allow-query-cache { 127.0.0.1; ${LAN_NET}; 192.168.1.0/24; 10.14.29.0/24; };
    dnssec-validation auto;
    max-cache-size 64m;
    max-ncache-ttl 300;
};

# View dedicada para rede WAN 10.14.29.0/24
# Usa EXCLUSIVAMENTE 10.14.8.20 e 10.1.6.222 como forwarders
# Garante resolução de sites que só existem nesses DNS internos
view "wan_view" {
    match-clients { 10.14.29.0/24; };
    recursion yes;
    forwarders { 10.14.8.20; 10.1.6.222; };
    forward only;
    allow-query { 10.14.29.0/24; };
    allow-query-cache { 10.14.29.0/24; };
};
EOF
...
zone "${LAN_REV_ZONE}.in-addr.arpa" {
    ...
    allow-query { 127.0.0.1; ${LAN_NET}; };
};

// Zona WPAD para autodetecção do proxy pelos clientes
zone "wpad.lan" {
    type master;
...
// Zonas NCSI — redireciona detectores de internet do Windows/Android/Apple
// para o gateway local, evitando falso "sem internet" no icone de rede.
...
ENVEOF
    ...
    # Atualizar apenas LAN_NET, preservando PANEL_PASSWORD com aspas intactas
    grep -v '^LAN_NET=' /etc/gateway-panel.env > /etc/gateway-panel.env.tmp
